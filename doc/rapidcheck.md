# RapidCheck property-based testing for ReBitcoin Core

## Concept

Property-based testing is experimentally being added to ReBitcoin Core with
...
If RapidCheck is installed, ReBitcoin Core will automatically run the
...
3. Configure ReBitcoin Core with RapidCheck.

    `cd` to the directory of your local rebitcoin repository and run
...
4. Build ReBitcoin Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_rebitcoin-crypto_gen.o
    CXX      test/test_rebitcoin-key_properties.o
    ```
...
That's it! You are now running property-based tests in ReBitcoin Core.
