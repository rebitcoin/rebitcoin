...
const char * const REBITCOIN_CONF_FILENAME = "rebitcoin.conf";
 ...
 * https://github.com/rebitcoin/rebitcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //rebitcoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "rebitcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ReBitcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ReBitcoin
    // Mac: ~/Library/Application Support/ReBitcoin
    // Unix: ~/.rebitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ReBitcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ReBitcoin";
#else
    // Unix
    return pathRet / ".rebitcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", REBITCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure ReBitcoin Core copyright is not removed by accident
    if (copyright_devs.find("ReBitcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The ReBitcoin Core developers";
    }
