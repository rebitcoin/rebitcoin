...
REBITCOIN_CONFIG_ALL="--disable-dependency-tracking --prefix=$BASE_BUILD_DIR/depends/$HOST --bindir=$BASE_OUTDIR/bin --libdir=$BASE_OUTDIR/lib"
...
DOCKER_EXEC ../configure --cache-file=config.cache $REBITCOIN_CONFIG_ALL $REBITCOIN_CONFIG || ( (DOCKER_EXEC cat config.log) && false)
END_FOLD

BEGIN_FOLD distdir
# Create folder on host and docker, so that `cd` works
mkdir -p "rebitcoin-$HOST"
DOCKER_EXEC make distdir VERSION=$HOST
END_FOLD

set +o errexit
cd "rebitcoin-$HOST" || (echo "could not enter distdir rebitcoin-$HOST"; exit 1)
set -o errexit
