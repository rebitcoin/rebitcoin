...
// rebitcoin includes.
#include <..\src\script\rebitcoinconsensus.h>
...
int main()
{
    std::cout << "rebitcoinconsensus version: " << rebitcoinconsensus_version() << std::endl;
    ...
    auto op0Result = rebitcoinconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, rebitcoinconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
